...
const ProfilePage: React.FC<ProfilePageProps> = ({ user, onBack }) => {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const [historyResult, favs] = await Promise.all([
          supabase
            .from('quiz_results')
            .select('*')
        ...
        setHistory((historyResult.data as QuizResultRecord[]) ?? []);
        setFavorites(favs);
      } catch (err: unknown) {
  ...
  const handleRemoveFavorite = async (fav: FavoriteGame) => {
    try {
      await removeFavorite(fav.steam_id, fav.game_id, fav.game_source);
    ...
    <div className="animate-results w-full max-w-4xl mx-auto pointer-events-auto">
        ...
        <div className="text-center md:text-left">
          ...
          <div className="flex gap-4 mt-2">
            <p className="text-blue-400 text-sm font-semibold">
              {history.length} quiz result{history.length !== 1 ? 's' : ''}
            </p>
      ...
      {loading && (
        <div className="py-20 text-center flex flex-col items-center animate-pulse">
          ...
          <p className="text-gray-500 font-black uppercase tracking-widest text-sm">Loading…</p>
        </div>
      )}
        ...
        <>
          {favorites.length === 0 ? (
            <div className="py-20 text-center">
              ...
              <p className="text-gray-700 text-xs mt-2">Save games using the ♥ button to see them here.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {favorites.map((fav) => (
                <div
                  key={fav.id}
                  className="steam-card rounded-xl overflow-hidden shadow-xl flex flex-col"
                >
                  {fav.game_image && (
                    <img
                      src={fav.game_image}
                  ...
                  <div className="p-4 flex flex-col flex-grow">
                    <p className="text-white text-sm font-black truncate mb-1">{fav.game_title}</p>
                    <p className="text-gray-500 text-xs uppercase tracking-wider mb-3">{fav.game_source}</p>
                    <button
                      onClick={() => handleRemoveFavorite(fav)}
                      className="mt-auto w-full py-1.5 bg-red-600/20 hover:bg-red-600/30 text-red-400 rounded text-xs font-bold transition-all"
                    >
                      Remove
                    </button>
                  </div>
